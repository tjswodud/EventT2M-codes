...
class TM2TMetrics(Metric):
    ...
    def __init__(self,
        ...
        # self.add_state("text_embeddings", default=torch.tensor(0.0), dist_reduce_fx=None)
        # self.add_state("recmotion_embeddings", default=torch.tensor(0.0), dist_reduce_fx=None)
        # self.add_state("gtmotion_embeddings", default=torch.tensor(0.0), dist_reduce_fx=None)
    ...
    def compute(self, sanity_flag):
        ...
        all_genmotions = all_genmotions.cpu().numpy()
        all_gtmotions = all_gtmotions.cpu().numpy()
        ...
        if count_seq > self.diversity_times:
            ...
            metrics["Diversity"] = torch.tensor(-1)
            metrics["gt_Diversity"] = torch.tensor(-1)
        return {**metrics}
    ...
    def update(
        ...
        # recmotion_embeddings = recmotion_embeddings.detach().cpu()
        # gtmotion_embeddings = gtmotion_embeddings.detach().cpu()
        # lengths = lengths.cpu()
        ...
        # if len(self.text_embeddings.shape) == 0:
        #     self.text_embeddings = text_embeddings
        #     self.recmotion_embeddings = recmotion_embeddings
        #     self.gtmotion_embeddings = gtmotion_embeddings
        # else:
        #     self.text_embeddings = torch.cat([self.text_embeddings, text_embeddings], dim=0)
        #     self.recmotion_embeddings = torch.cat([self.recmotion_embeddings, recmotion_embeddings], dim=0)
        #     self.gtmotion_embeddings = torch.cat([self.gtmotion_embeddings, gtmotion_embeddings], dim=0)
